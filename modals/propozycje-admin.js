...
const { id_kanal_sp_admin } = require('../assets/config.json');
...
module.exports = {
  ...
  async execute(modal,bot){
    ...
    const kanal = await bot.channels.fetch(id_kanal_sp_admin);
    kanal.send({embeds: [embed]});
    await modal.deferReply({ ephemeral: true })
    modal.followUp({ content: 'Wysłano propozycję do administracji:' + Formatters.codeBlock('markdown', text), ephemeral: true })
