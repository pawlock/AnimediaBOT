...
const { id_kanal_r_animedia } = require('../assets/config.json');
...
module.exports = {
  ...
  async execute(modal,bot){
    ...
    const kanal = await bot.channels.fetch(id_kanal_r_animedia);
    kanal.send({embeds: [embed]});
    await modal.deferReply({ ephemeral: true })
    modal.followUp({ content: 'Wysłano twoją aplikację do ANIMEDII!', ephemeral: true })
