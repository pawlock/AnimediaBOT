...
const { id_kanal_s_admin } = require('../assets/config.json');
...
module.exports = {
  ...
  async execute(modal,bot){
    ...
    const kanal = await bot.channels.fetch(id_kanal_s_admin);
    kanal.send({embeds: [embed]});
    await modal.deferReply({ ephemeral: true })
    modal.followUp({ content: 'Wysłano skargę do administracji:' + Formatters.codeBlock('markdown', text), ephemeral: true })
